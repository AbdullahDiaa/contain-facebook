...
async function sendJailedDomainsToMAC () {
  try {
    return await browser.runtime.sendMessage(MAC_ADDON_ID, {
      ...
      urls: YOUTUBE_DOMAINS.map((domain) => {
        return `https://${domain}/`;
      })
    });
  } catch (e) {
...
async function clearYouTubeCookies () {
  ...
  let macAssignments = [];
  if (macAddonEnabled) {
    const promises = YOUTUBE_DOMAINS.map(async youtubeDomain => {
      const assigned = await getMACAssignment(`https://${youtubeDomain}/`);
      return assigned ? youtubeDomain : null;
    });
    macAssignments = await Promise.all(promises);
  }

  YOUTUBE_DOMAINS.map(async youtubeDomain => {
    const youtubeCookieUrl = `https://${youtubeDomain}/`;

    // dont clear cookies for youtubeDomain if mac assigned (with or without www.)
    if (macAddonEnabled &&
        (macAssignments.includes(youtubeDomain) ||
         macAssignments.includes(`www.${youtubeDomain}`))) {
      return;
    }

    containers.map(async container => {
      const storeId = container.cookieStoreId;
      if (storeId === youtubeCookieStoreId) {
        // Don't clear cookies in the YouTube Container
        return;
      }
      ...
      cookies.map(cookie => {
        browser.cookies.remove({
          name: cookie.name,
          url: youtubeCookieUrl,
          storeId
        });
      });
      // Also clear Service Workers as it breaks detecting onBeforeRequest
      await browser.browsingData.remove({hostnames: [youtubeDomain]}, {serviceWorkers: true});
    });
  });
}
...
function shouldContainInto (url, tab) {
  ...
  if (isYouTubeURL(url)) {
    if (tab.cookieStoreId !== youtubeCookieStoreId) {
      // YouTube-URL outside of YouTube Container Tab
      // Should contain into YouTube Container
      return youtubeCookieStoreId;
    }
  } else if (tab.cookieStoreId === youtubeCookieStoreId) {
    // Non-YouTube-URL inside YouTube Container Tab
